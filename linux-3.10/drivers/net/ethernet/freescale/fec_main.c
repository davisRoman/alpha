...
static void set_ksz8081_50MHz(struct phy_device *phy);
static void set_ksz8081_50MHz(struct phy_device *phy)
{
    int value;
    udelay(2000);
    ...
    value = phy_read(phy, 0x1f);
	value |= (1 << 7);
	phy_write(phy, 0x1f, value);
    value = phy_read(phy, 0x1f);
}
...
fec_enet_open(struct net_device *ndev)
{
	...
	msleep(1);
	phyid0 = phy_read(fep->phy_dev, 0x2);
	phyid1 = phy_read(fep->phy_dev, 0x3);
	/* If it is the KSZ8081 PHY, configure it to 50MHz mode  */
	if(((phyid0 & 0xFFFF) == 0x0022) && ((phyid1 & 0xFFF0) == 0x1560))
	    set_ksz8081_50MHz(fep->phy_dev);
