...
static int imx6_pcie_deassert_core_reset(struct pcie_port *pp)
{
	...
	if (gpio_is_valid(imx6_pcie->power_on_gpio)) {
        if (gpio_request(imx6_pcie->power_on_gpio, "power_on_gpio") == 0) {
            gpio_direction_output(imx6_pcie->power_on_gpio, 1);
            gpio_free(imx6_pcie->power_on_gpio);
        }
    }
	...
	if (gpio_is_valid(imx6_pcie->reset_gpio)) {
        if (gpio_request(imx6_pcie->reset_gpio, "reset_gpio") == 0) {
            gpio_direction_output(imx6_pcie->reset_gpio, 0);
            mdelay(1);
            gpio_set_value_cansleep(imx6_pcie->reset_gpio, 1);
            gpio_free(imx6_pcie->reset_gpio);
        }
	}
