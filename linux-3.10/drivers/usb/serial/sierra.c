...
/* Sierra driver - kernel 3.1x */
#define DRIVER_VERSION "v.1.7.52"
...
static const struct usb_device_id id_table [] = {
	...
	{ USB_DEVICE(0x1199, 0x0300) },	/* Sierra Wireless SL3/5 */
...
static int sierra_remove_sysfs_attrs(struct usb_serial_port *port)
{
    ...
    if (port->serial->num_ports > 0) {
		...
		if (port_tmp)
			 ...
			 */
			printk("%s usb_get_serial_port_data(port_tmp) = %p\n", __func__, usb_get_serial_port_data(port_tmp));
