...
{
/* TSVE 30/1024 best ECC strength */
    int ecc_strength;
    ecc_strength = 30;
    return ecc_strength;
#if 0   /*old stadard version for 3.10.53 */
...
static bool mxs_nand_erased_page(struct mtd_info *mtd, struct nand_chip *nand,
				 uint8_t *buf, int chunk, int page,struct bch_geometry *geo)
{
	int gf_len = geo->gf_len;
    struct gpmi_nand_data *gpmi_nand = nand->priv;
	unsigned int flip_bits = 0, flip_bits_noecc = 0;
	unsigned int threshold;
	unsigned int ecc_chunkn_size = geo->ecc_chunk_size;
	unsigned int base = ecc_chunkn_size * chunk;
	uint32_t *dma_buf = (uint32_t *)gpmi_nand->data_buffer_dma;
	int i;

	int ecc_strength = get_ecc_strength(gpmi_nand);
	threshold = gf_len / 2;
	...
	nand->read_buf(mtd, (uint8_t *)dma_buf, mtd->writesize);
	...
	printk("The page(%d) is an erased page(%d,%d,%d,%d).\n", page, chunk, threshold, flip_bits, flip_bits_noecc);
				...
				uint8_t *buf, int oob_required, int page)
{
	...
	for (i = 0; i < nfc_geo->ecc_chunk_count; i++, status++) {
		...
		if (*status == STATUS_UNCORRECTABLE) {
			if (mxs_nand_erased_page(mtd, chip,
						 payload_virt, i, page,nfc_geo))
				break;
